...
export default function NewQuotation(props) {
    const classes = useStyles();

    const [quotationId, setQuotationId] = useState(props.id)
    ...
    const [submitUrl,setSubmitUrl] = useState("/api/w/quotations/")
    ...
    const loadQuotationDetails = function (){
        if (!quotationId || quotationId == 0)
            return
        setIsLoading(true)
        fetch(`/api/w/quotations/${quotationId}/`,{
            method:"GET",
            headers:{
        ...
        .then(res=> res.json())
        .then(quotation => {
            setSubmitUrl(`/api/w/quotations/${quotationId}/`)
            setSubmitMethod('PUT')
            setCustomerName(quotation.name)
            setPhoneNo(quotation.phoneNumber)
            setAddress(quotation.address)
            setIsLoading(false)
            
            var quotationItemsList = []
            var quotationChargesList = []

            quotation.QuotationItems.map(({item,qty,price,total})=>{
                quotationItemsList.push({
                    itemName:item,
                    qty,
                    price,
                    amount:total
            ...
            quotation.QuotationCharges.map(({chargeName,chargeAmount})=>{
                quotationChargesList.push({
                    chargeName,chargeAmount
                })
            })

            setOrderItems(quotationItemsList)
            setCharges(quotationChargesList)
        })
    }
    ...
    const submitQuotation = function () {
        ...
        if (orderItems.length == 0) {
            setAlert({
                message: "Atleast one item should be added to the quotation.",
                color: "error"
            })
            setIsAlert(true)
            return
        ...
        fetch(submitUrl, {
            ...
            setAlert({
                message: "quotation Saved Successfuly.",
                color: "success"
            })
            refreshPage()
            setIsLoading(false)
    ...
    return (
        <div>
            ...
            <Title>
            {quotationId ? "Edit" : "New"} Quotation
            </Title>
